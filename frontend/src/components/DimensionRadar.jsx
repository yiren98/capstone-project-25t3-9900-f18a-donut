...
export default function DimensionRadar({
  ...
  const colors = ["#F6C945","#8BC5BF","#7F86C6","#C993C7","#F39B7F","#6DB1FF","#9DD67A","#E6B8A2","#B9A1FF","#FFC4A1","#8FD3FF","#F2B2CE"];
    ...
    try {
      if (level === 0) {
        ...
        setItems((data || []).map((d,i)=>({
          name: d.name, value: Number(d.count||0), color: colors[i % colors.length],
        })).filter(d=>d.value>0));
      } else {
  ...
  return (
    <div
      ...
      style={{ width:"100%", maxWidth: widthPx, height: heightPx, background:"rgba(255,255,255,0.7)" }}
    >
      ...
      <div className="grid"
           style={{ gridTemplateColumns: "minmax(240px,1fr) 1.4fr", gap:"10px", height:"calc(100% - 28px)" }}>
              ...
              {sectors.map((d,i)=>{
                ...
                return (
                  <text key={i} x={tx} y={ty} textAnchor="middle" dominantBaseline="central"
                        fontSize="027" fill="#333" style={{ pointerEvents:"none" }}>
                    {d.value}
                  </text>
                );
              })}
        ...
        <div className="h-full flex items-center">{loading ? <div className="text-[12px] text-neutral-500">Loading…</div> : legend}</div>
      </div>
    </div>
  );
}
