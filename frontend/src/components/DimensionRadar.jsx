...
export default function DimensionRadar({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      const maxLeftAllowed = Math.max(200, Math.min(leftMax, W - rightMin - gapPx));
      setLeftSize(Number.isFinite(maxLeftAllowed) ? Math.max(200, maxLeftAllowed) : leftMax);
    });
    ro.observe(el);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (level === 0) {
        ...
        setItems((data || [])
          .map((d, i) => ({ name: d.name, value: Number(d.count || 0), color: colors[i % colors.length] }))
          .filter((d) => d.value > 0));
      } else {
        const data = await getSubthemeCounts({ year, month, dimension: focusDim });
        setItems((data || [])
          .map((d, i) => ({ name: d.name, value: Number(d.count || 0), color: colors[i % colors.length] }))
          .filter((d) => d.value > 0));
      }
  ...
  useEffect(() => {
    if (!selectedDimension) { setLevel(0); setFocusDim(""); setActiveSub(""); return; }
    setLevel(1); setFocusDim(selectedDimension); setActiveSub(selectedSubtheme || "");
  ...
  const arcPath = (cx, cy, r0, r1, a0, a1) => {
    ...
    const x0 = cx + r1 * c(a0), y0 = cy + r1 * s(a0);
    const x1 = cx + r1 * c(a1), y1 = cy + r1 * s(a1);
    const x2 = cx + r0 * c(a1), y2 = cy + r0 * s(a1);
    const x3 = cx + r0 * c(a0), y3 = cy + r0 * s(a0);
  ...
  const clickDimension = (dimName) => {
    ...
    if (selectedDimension === dimName) {
      ...
      setLevel(0); setFocusDim(""); setActiveSub("");
    } else {
      onFilterChange?.({ dimension: dimName, subtheme: "" });
      setLevel(1); setFocusDim(dimName); setActiveSub("");
    }
  };
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const top = SC_GUTTER + (scrollTop / (scrollHeight - clientHeight)) * (available - height);
      setThumb({ top, height });
    };
    const onScroll = () => {
      update(); setScrolling(true);
      ...
      >
        {items.map((d, i) => {
          ...
          return (
            <button
              key={`${d.name}-${i}`}
              onClick={() => (level === 0 ? clickDimension(d.name) : clickSubtheme(d.name))}
              className={`flex items-center gap-1 rounded-lg border px-2 py-1.5 shadow-sm text-left hover:shadow transition
            ...
            >
              ...
              <span className="text-[11px] text-neutral-700 truncate">{d.name}</span>
              <span className="ml-auto text-[11px] font-semibold tabular-nums text-neutral-800">{d.value}</span>
            </button>
          );
        })}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-stretch" style={{ gap: gapPx, height: "calc(var(--card-h) - 72px)" }}>
        <div className="relative shrink-0" style={{ width: leftSize, height: "100%", overflow: "hidden" }}>
          {loading ? (
            <div className="w-full h-full flex items-center justify-center text-[12px] text-neutral-500">Loading…</div>
          ) : items.length === 0 ? (
            <EmptyState style={{ width: "100%", height: "100%" }}>
              {selectedSubtheme
                ...
                : `No data${year ? ` · ${year}` : ""}${month ? `-${month}` : ""}.`}
            </EmptyState>
          ) : (
            <svg width={leftSize} height={"100%"}>
              <g opacity="0.28" stroke="#eee7dc">
                {[0.25, 0.5, 0.75, 1].map((t, i) => (
                  <circle key={i} cx={cx} cy={cy} r={rMin + (rMax - rMin) * t} fill="none" />
                ))}
              ...
              <g>
                {sectors.map((d, i) => {
                  const a = (d.start + d.end) / 2, rr = d.rOut + 12;
                  const tx = cx + rr * Math.cos(a), ty = cy + rr * Math.sin(a);
                  return (
                    <text key={i} x={tx} y={ty} textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#333" style={{ pointerEvents: "none" }}>
                      {d.value}
                    </text>
                  );
                })}
              ...
              {level === 1 && items.length > 0 && !loading && (
                <foreignObject x="8" y={H - 32} width="24" height="24">
                  <button
                    onClick={backToTop}
                    className="w-6 h-6 rounded-md border border-neutral-300/70 hover:bg-neutral-50 text-[14px]"
                    title="Back"
                    style={{ cursor: "pointer" }}
                  >⮌</button>
                </foreignObject>
        ...
        <div className="relative h-full min-w-0 flex-1">
          {loading ? (
            <div className="h-full flex items-center justify-center text-[12px] text-neutral-500">Loading…</div>
          ) : items.length === 0 ? (
            <EmptyState style={{ width: "100%", height: "100%" }}>Try another month/dimension.</EmptyState>
          ) : (
            legendGrid
          )}
          ...
          {!loading && items.length > 0 && thumb.height > 0 && !isTouch && (
            ...
            >
              <div
                ...
                style={{ width: SCROLL_WIDTH, background: SC_TRACK_COLOR, boxShadow: "inset 0 0 1px rgba(255,255,255,0.04)" }}
              />
              <div
                className="absolute right-0 rounded-full"
                style={{ width: SCROLL_WIDTH, top: thumb.top, height: thumb.height, background: SC_THUMB_COLOR, boxShadow: `inset 0 0 0 1px ${SC_THUMB_BORDER}` }}
              />
            </div>
          )}
        </div>
