...
const SAMPLE = {
  ...
  section: {
    executive_briefing: {
      ...
      note:
        "These recommendations are based on available data and trends. Continuous monitoring of regulatory developments and competitor movements is advised."
    }
  }
};
    ...
    content: (
      <div className="grid md:grid-cols-2 gap-3">
        {["risks", "opportunities"].map((t) => (
          <div key={t} className="rounded-xl border border-[#eee7db] bg-white/60 p-3">
            <div className="text-sm font-medium mb-2">
              {t === "risks" ? "Risks" : "Opportunities"}
            </div>
            <ul className="space-y-2 text-sm text-neutral-800">
              {SAMPLE.section.executive_briefing.risks_and_opportunities[t].map((x, i) => (
                <li key={i} className="pl-4 relative">
                  <span className="absolute left-0 top-[9px] h-1.5 w-1.5 rounded-full bg-neutral-400" />
                  <span className="font-medium">{x.name}:</span> {x.description}
                </li>
              ))}
            </ul>
          </div>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        {SAMPLE.section.executive_briefing.actionable_recommendations.map((a, i) => (
          <div key={i} className="p-3 rounded-xl bg-white shadow-sm border border-[#eee7db]">
            <div className="text-[15px] font-medium mb-1">{a.recommendation}</div>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        {sections.slice(page * pageSize, page * pageSize + pageSize).map((sec, i) => (
          <div key={i}>
